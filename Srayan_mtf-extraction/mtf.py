...
class MTF:
    ...
    def GetEdgeSpreadFunction(imgArr, edgePoly, verbose=Verbosity.NONE):
        ...
        for y in range(Y):
            distance[y, :] = (edgePoly[0] * column - (y + 0.5) + edgePoly[1]) / np.sqrt(
                edgePoly[0] * edgePoly[0] + 1
            )
        ...
        if verbose == Verbosity.BRIEF:
            print(
                "Raw ESF [done] (Distance from {0:2.2f} to {1:2.2f})".format(
                    sign * distances[0], sign * distances[-1]
                )
            )
    ...
    def GetEdgeSpreadFunctionCrop(imgArr, verbose=Verbosity.NONE):
        ...
        esfRaw = MTF.SafeCrop(
            esfValues, esfDistances, head - 1.2 * width, tail + 1.2 * width
        )
        ...
        if verbose == Verbosity.BRIEF:
            print(
                "ESF Crop [done] (Distance from {0:2.2f} to {1:2.2f})".format(
                    esfRaw.x[0], esfRaw.x[-1]
                )
            )
    ...
    def SimplifyEdgeSpreadFunction(esf, verbose=Verbosity.NONE):
        ...
        if verbose == Verbosity.BRIEF:
            print(
                "ESF Simplification [done] (Size from {0:d} to {1:d})".format(
                    np.size(esf.x), np.size(distances)
                )
            )
            ...
            fig.canvas.manager.set_window_title(
                "ESF Simplification (Size from {0:d} to {1:d})".format(
                    np.size(esf.x), np.size(distances)
                )
            )
            (ax1, ax2) = plt.subplots(2)
    ...
    def GetMTF(lsf, verbose=Verbosity.NONE):
        ...
        if verbose == Verbosity.BRIEF:
            ...
            fig = pylab.gcf()
            fig.canvas.manager.set_window_title(
                "MTF ({0:2.2f}% at Nyquist)".format(valueAtNyquist)
            )
            (ax1) = plt.subplots(1)
    ...
    def CalculateMtf(imgArr, verbose=Verbosity.NONE):
        ...
        if verbose == Verbosity.BRIEF:
            print(
                "MTF at Nyquist:{0:0.2f}%, Transition Width:{1:0.2f}".format(
                    mtf.mtfAtNyquist, esf.width
                )
            )
            ...
            ax1.plot(x, y, color="red")
            ax1.axis("off")
            ax2.plot(esf.rawESF.x, esf.rawESF.y, esf.interpESF.x, esf.interpESF.y)
            ...
            ax4.set_title(
                "MTF at Nyquist:{0:0.2f}%\nTransition Width:{1:0.2f}".format(
                    mtf.mtfAtNyquist, esf.width
                )
            )
            ax4.grid(True)
